...
class ModelCouncil extends EventEmitter {
  ...
  async startSession(task, options = {}) {
    ...
    // Emit event
    this.emit('session:started', { sessionId, task, modelCount: ollamaModels.length });
  ...
  async runCouncilMeeting(sessionId) {
    ...
    this.emit('session:phase', { sessionId, phase: 'gathering_proposals', modelCount: models.length });
    ...
    this.emit('session:proposals_complete', { sessionId, proposalCount: proposals.length });
    ...
    if (this.config.enableDebates && proposals.length >= 2) {
      ...
      this.emit('session:phase', { sessionId, phase: 'debating' });
      ...
      this.emit('session:debates_complete', { sessionId, debateCount: debates.length });
    }

    // Phase 3: Build consensus
    console.log(`[ModelCouncil] Phase 3: Building consensus...`);
    this.emit('session:phase', { sessionId, phase: 'consensus_building' });
    ...
    this.emit('session:consensus_reached', { sessionId, consensus });
  ...
  async queryModel(sessionId, modelName, prompt) {
    ...
    try {
      ...
      // Emit event
      this.emit('session:proposal', { sessionId, proposal });
  ...
  async generateDebates(sessionId, proposals) {
    ...
    for (const [model1, model2] of interestingPairs.slice(0, 3)) { // Limit to 3 debates
      ...
      this.emit('session:debate', { sessionId, debate });
    }
