...
class MultiLLMRouter {
  ...
  async complete(request) {
    ...
    try {
      ...
      console.log(`[MultiLLMRouter] Routing to ${provider} (strategy: ${this.config.strategy})`);
  ...
  _selectSmart(available, request) {
    ...
    switch (taskType) {
      ...
      case 'reasoning':
        // Complex reasoning: GPT-4 > Claude > DeepSeek
        if (available.includes('openai')) {
          return 'openai'; // GPT-4 best at reasoning
        }
        if (available.includes('anthropic')) {
          return 'anthropic';
        }
