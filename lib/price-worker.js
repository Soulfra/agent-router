 ...
 *
 * NOW WITH SUB-AGENT CONTEXT:
 * - Can be spawned as a sub-agent with parent context
 * - Uses platform database router when available
 * - Can communicate back to parent agent
 * - Inherits platform/device information
 */
...
class PriceWorker {
  constructor(options = {}) {
    // SUB-AGENT CONTEXT: Attach context if provided
    this.context = options.subAgentContext || null;
    this.isSubAgent = !!this.context;

    // Platform DB Router (if available)
    this.dbRouter = options.dbRouter || null;

    // Database connection
    // If we have a router, use it; otherwise fall back to direct pool
    if (this.dbRouter) {
      this.db = null; // Will use router for queries
      console.log('[PriceWorker] Using platform database router');
    } else {
    ...
    this.pricingSource = options.pricingSource || new PricingSource({
      db: this.db,
      dbRouter: this.dbRouter,
  ...
  start() {
    ...
    const agentInfo = this.isSubAgent
      ? `[Sub-Agent ${this.context.agentId} | depth ${this.context.contextDepth}]`
      : '[Root Agent]';

    console.log(`[PriceWorker] ${agentInfo} Starting price worker...`);
    ...
    if (this.isSubAgent) {
      console.log(`[PriceWorker] Platform: ${this.context.platform || 'default'}, Device: ${this.context.deviceType || 'default'}`);
    }
    ...
    if (this.isSubAgent && this.context.parentAgentId) {
      this.context.messageParent({
        type: 'worker_started',
        worker: 'PriceWorker',
        stats: this.getStats()
      }).catch(err => console.error('[PriceWorker] Failed to notify parent:', err.message));
    }
  }
  ...
  /**
   * Execute database query (router-aware)
   */
  async query(sql, params = []) {
    if (this.dbRouter) {
      // Use router with context
      const contextConfig = this.context ? this.context.getDatabaseConfig() : {};
      return await this.dbRouter.query(sql, params, contextConfig);
    } else if (this.db) {
  ...
  async calculate24hMetrics(symbol, currentPrice) {
    try {
      ...
      if (result.rows && result.rows.length > 0) {
        ...
        const volumeResult = await this.query(`
