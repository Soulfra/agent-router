-- Vanity Handles System - Tables Only (Fixed for existing schema)
-- The handle columns were already added in 032_vanity_handles.sql
-- This migration creates the supporting tables
...
CREATE TABLE IF NOT EXISTS handle_reservations (
  ...
  -- Ownership (changed user_id to id to match users table)
  reserved_for_user_id INTEGER REFERENCES users(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_handle_reservations_status ON handle_reservations(status);
CREATE INDEX IF NOT EXISTS idx_handle_reservations_type ON handle_reservations(reservation_type);
...
CREATE INDEX IF NOT EXISTS idx_handle_history_user ON handle_history(user_id, changed_at DESC);
CREATE INDEX IF NOT EXISTS idx_handle_history_handle ON handle_history(new_handle);
...
CREATE INDEX IF NOT EXISTS idx_blocked_handles_reason ON blocked_handles(block_reason);

-- ============================================================================
-- FUNCTIONS (Updated to use 'id' instead of 'user_id')
-- ============================================================================
...
-- Set user handle (Updated to use 'id')
...
-- Get user by handle (Updated to use 'id')
