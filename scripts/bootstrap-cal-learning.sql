-- Bootstrap Cal's Learning Environment
-- Creates debugging-mastery path and all 10 lessons
-- Run with: psql -d calos -U matthewmauer -f scripts/bootstrap-cal-learning.sql

-- First check if domain_portfolio table exists (required for learning_paths foreign key)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'domain_portfolio') THEN
    -- Create minimal domain_portfolio table if it doesn't exist
    ...
    -- Insert a placeholder domain for learning paths
    INSERT INTO domain_portfolio (domain_id, domain_name)
    VALUES ('00000000-0000-0000-0000-000000000001', 'learning-platform');
  END IF;
END $$;

-- Create debugging-mastery learning path
INSERT INTO learning_paths (
  ...
  estimated_hours,
  skills_learned,
  xp_reward_per_lesson,
  status
) VALUES (
  '95518df4-58ab-498f-ba4b-8a9344de3c76',  -- Fixed UUID for easy reference
  '00000000-0000-0000-0000-000000000001',  -- learning-platform domain
  'Debugging Mastery',
  'debugging-mastery',
  'Learn OSS diagnostic tools from grep to Guardian-level automation',
  'Master the essential debugging tools used in professional software development. Progress from basic pattern matching with grep to building autonomous monitoring systems like Guardian Agent.',
  'beginner',
  10,
  5.0,
  ARRAY['debugging', 'shell-scripting', 'grep', 'sed', 'jq', 'diagnostics', 'automation'],
  200,
  'published'
...
-- Now insert all 10 lessons (same as create-debugging-lessons.sql but with path creation first)
